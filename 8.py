...
df['r'] = (df['Close '] - df['Open ']) / df['Open '] * 100
display(df.head())

from statsmodels.tsa.stattools import acf, pacf

# Reduce nlags to a value less than half of the sample size
max_lags = 10  # Or any value less than len(df['r']) // 2
acf_values = acf(df['r'], nlags=max_lags)
pacf_values = pacf(df['r'], nlags=max_lags)
...
# Simulate stock-like returns (AR(1) process: X_t = 0.7 * X_{t-1} + noise)
np.random.seed(42)
n = 200
returns = np.zeros(n)
for t in range(1, n):
    returns[t] = 0.7 * returns[t-1] + np.random.normal(0, 0.1)

# Step 1: Center the data (subtract mean)
returns = returns - np.mean(returns)

# Step 2: Compute ACF manually
def compute_acf(data, max_lags):
    n = len(data)
    acf_values = np.zeros(max_lags + 1)
    variance = np.sum(data ** 2) / n  # Denominator for ACF
    for k in range(max_lags + 1):
        if k == 0:
            acf_values[k] = 1.0  # ACF at lag 0 is always 1
        else:
            # Sum of products for lag k
            cov = np.sum(data[:-k] * data[k:]) / (n - k)
            acf_values[k] = cov / variance
    return acf_values

# Step 3: Compute PACF manually using linear regression
def compute_pacf(data, max_lags):
    n = len(data)
    pacf_values = np.zeros(max_lags + 1)
    pacf_values[0] = 1.0  # PACF at lag 0 is 1
    for k in range(1, max_lags + 1):
        # Create lagged matrix for lags 1 to k
        X = np.zeros((n - k, k))
        for j in range(k):
            X[:, j] = data[k-j-1:n-j-1]  # Lags 1, 2, ..., k
        y = data[k:]  # Target variable
        # Solve linear regression: y = b1*lag1 + b2*lag2 + ... + bk*lagk
        try:
            coeffs = np.linalg.lstsq(X, y, rcond=None)[0]
            pacf_values[k] = coeffs[-1]  # Coefficient of lag k is PACF
        except np.linalg.LinAlgError:
            pacf_values[k] = 0  # Handle singular matrix
    return pacf_values

# Compute ACF and PACF
max_lags = 25
acf_values = compute_acf(returns, max_lags)
pacf_values = compute_pacf(returns, max_lags)

# Step 4: Plot ACF and PACF
lags = np.arange(max_lags + 1)
confidence_interval = 1.96 / np.sqrt(n)  # 95% confidence band
...
plt.axhline(y=confidence_interval, linestyle="--", color="gray")
plt.axhline(y=-confidence_interval, linestyle="--", color="gray")
plt.title("Manual ACF of Stock Returns")
plt.xlabel("Lag")
...
plt.axhline(y=confidence_interval, linestyle="--", color="gray")
plt.axhline(y=-confidence_interval, linestyle="--", color="gray")
plt.title("Manual PACF of Stock Returns")
plt.xlabel("Lag")
...
# Print sample ACF and PACF values
print("ACF values:")
for k, val in enumerate(acf_values):
    print(f"Lag {k}: {val:.4f}")
print("\nPACF values:")
for k, val in enumerate(pacf_values):
    print(f"Lag {k}: {val:.4f}")
